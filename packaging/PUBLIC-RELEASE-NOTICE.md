...
The in-app updater replaces only the versioned DSH runtime; it cannot replace the launcher executable or its UI
integration. Users of launcher 1.1.0, 1.2.0, or 1.2.1 must install the complete 1.3.0 archive to receive the General
settings update entry. The archive does not contain user configuration from `%USERPROFILE%\.dsh`.
