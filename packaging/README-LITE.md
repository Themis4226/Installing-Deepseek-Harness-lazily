# DSH Desktop Launcher Lite 1.3.0（Windows x64）
...
1.3.0 在 DSH 的“设置 → 通用设置”中加入与其他设置行一致的“软件更新”入口；它调用的仍是
启动器原生更新器。旧版顶部“帮助”菜单已移除，更新入口统一放在通用设置中。
...
1.3.0 可在“设置 → 通用设置 → 软件更新”中下载经过大小与 SHA-256 校验的版本化 DSH 运行时。
更新会先进入独立目录，启动器只在新版本通过进程、URL 与 HTTP 2xx 健康检查后确认切换；失败会自动
回到上一版。运行时更新不会替换 EXE，也不会删除上述用户数据。它不是差分补丁，仍需下载完整运行时。

“软件更新”页面插件保存在 `launcher-integration`，由启动器的受限预加载桥映射；它不会复制或写入
随包或已下载的 DSH 运行时目录。

因此，1.1.0、1.2.0 和 1.2.1 升级到 1.3.0 时，都需要下载并完整解压本包；旧启动器不能通过
运行时更新获得新的原生消息桥。发布 ZIP 中不包含制作者的缓存、日志、账号信息或 `.dsh` 配置。
