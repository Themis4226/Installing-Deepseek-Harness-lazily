# DSH Desktop Launcher Lite 1.4.4（Windows x64）

这是一个非官方的 DeepSeek Harness (`dsh`) Windows 图形启动器。它不隶属于 DeepSeek，亦不代表
DeepSeek 对本启动器的认可或背书。

1.4.4 随包提供未经改动的官方 DSH `0.1.7-rc.2`，并继续保留针对
`REQUEST_EXTENSION` 对话失败的启动器配置。图标保持不变。
官方 0.1.7 将会话格式升级为 V4，并导入旧版设置；新版本写入的会话和设置不保证能由旧 DSH 读取。
...
- 初始 DSH：固定为 `@deepseek-ai/dsh@0.1.7-rc.2`
- 仅绑定动态分配的 `127.0.0.1` 本机端口。
- DSH 仍处于 developer preview，升级前需要重新验证兼容性。

建议先备份用户目录。第三方插件可能需要适配，详见官方
`dsh-v0.1.7-rc.1` 和 `dsh-v0.1.7-rc.2` 发布说明。
