...
const FORBIDDEN_LAUNCHER_PACKAGES = [
...
async function inspectArchive(archivePath, tarPath) {
  ...
  const forbiddenLauncherPackages = FORBIDDEN_LAUNCHER_PACKAGES.map(
  ...
  for (const rawName of rawNames) {
    ...
    if (forbiddenLauncherPackages.some((root) => folded === root || folded.startsWith(`${root}/`))) {
      fail('archive contains a launcher-only package inside the DSH runtime tree')
    }
