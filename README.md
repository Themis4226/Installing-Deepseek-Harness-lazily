...
1.1.0 启动器没有更新器；1.2.0 的更新器只更新 DSH 运行时，不能替换启动器 EXE。由于 1.2.1 修复了
1.2.0 会额外打开默认浏览器的问题，这两个旧版本都必须手动下载一次 1.2.1 完整包。完整包包含启动器
和初始 DSH 运行时，但不包含 Node.js。不要把 EXE 单独移动到桌面；需要桌面入口时，请创建快捷方式。

## 1.2.1 的程序内运行时更新

启动器成功安装到 1.2.1 后，可以在窗口菜单中选择 **Help → 检查更新…**。启动器也会在正常启动后
...
1.2.1 通过 `--no-open` 启动 DSH，因此正常情况下只显示内嵌桌面窗口，不会再额外打开系统默认浏览器。
当前更新器不更新启动器 EXE；未来如有启动器修复，发布说明会明确是否需要重新下载完整包。
